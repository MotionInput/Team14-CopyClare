...
class Model:
        ...
        try:
            with open(csv_name,mode="r") as f:
                ...
                for row in reader:
                    ...
                    if row_num % 7 == 0: 
                        data_set.append([])
                    data_set[-1].append({"landmark_num":row[0],"coordinates":row[1]})
                return data_set
    ...
    def get_video_angle_data(self,data_set,landmark_num1,landmark_num2,landmark_num3):
        video_angle_data =[]
        for data in data_set:
            for dict in data:
                ...
                angle = math.degrees(math.acos(dot_product/np.linalg.norm(v1)/np.linalg.norm(v2)))   
